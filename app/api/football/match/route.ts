...
export async function GET(request: Request) {
  ...
  try {
    ...
    let statistics: Array<{ team: { id: number; name: string; logo: string | null }; statistics: Array<{ type: string; value: string | number | null }> }> = [];
    if (['FT', 'AET', 'PEN'].includes(match.status) && process.env.API_FOOTBALL_KEY) {
      ...
      if (response.ok) statistics = ((await response.json()) as { response?: typeof statistics }).response ?? [];
    }
