...
export async function ensureFootballSchema() {
  ...
  await db.batch([
    ...
    db.prepare(`CREATE TABLE IF NOT EXISTS fixture_player_statistics (fixture_id INTEGER NOT NULL, team_id INTEGER NOT NULL, player_id INTEGER NOT NULL, player_name TEXT NOT NULL, position TEXT, minutes INTEGER, rating REAL, captain INTEGER, substitute INTEGER, offsides INTEGER, shots_total INTEGER, shots_on INTEGER, goals INTEGER, goals_conceded INTEGER, assists INTEGER, saves INTEGER, passes_total INTEGER, passes_key INTEGER, passes_accuracy REAL, tackles INTEGER, blocks INTEGER, interceptions INTEGER, duels INTEGER, duels_won INTEGER, dribbles_attempts INTEGER, dribbles_success INTEGER, dribbled_past INTEGER, fouls_drawn INTEGER, fouls_committed INTEGER, yellow_cards INTEGER, red_cards INTEGER, penalties_won INTEGER, penalties_committed INTEGER, penalties_scored INTEGER, penalties_missed INTEGER, penalties_saved INTEGER, updated_at TEXT NOT NULL, PRIMARY KEY (fixture_id, team_id, player_id))`),
    db.prepare(`CREATE TABLE IF NOT EXISTS fixture_detail_imports (fixture_id INTEGER NOT NULL, detail_class TEXT NOT NULL, status TEXT NOT NULL, message TEXT, attempted_at TEXT NOT NULL, PRIMARY KEY (fixture_id, detail_class))`),
    ...
    db.prepare(`CREATE TABLE IF NOT EXISTS fixture_availability_snapshots (id INTEGER PRIMARY KEY AUTOINCREMENT, fixture_id INTEGER NOT NULL, lineups TEXT NOT NULL, injuries TEXT NOT NULL, captured_at TEXT NOT NULL)`),
    ...
    db.prepare(`CREATE INDEX IF NOT EXISTS idx_fixture_player_statistics_fixture ON fixture_player_statistics (fixture_id)`),
    db.prepare(`CREATE INDEX IF NOT EXISTS idx_fixture_player_statistics_player ON fixture_player_statistics (player_id, fixture_id)`),
    db.prepare(`CREATE INDEX IF NOT EXISTS idx_fixture_detail_imports_class_status ON fixture_detail_imports (detail_class, status)`),
    db.prepare(`CREATE INDEX IF NOT EXISTS idx_fixture_lineups_fixture ON fixture_lineups (fixture_id)`),
    db.prepare(`CREATE INDEX IF NOT EXISTS idx_fixture_availability_fixture_captured ON fixture_availability_snapshots (fixture_id, captured_at)`),
