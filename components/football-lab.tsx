...
function CompetitionDatabase({ feed, loading }: { feed: IntelligenceFeed | null; loading: boolean }) {
  ...
  const loadTeam = (teamId: number) => {
    ...
    fetch(`/api/football/team?team=${teamId}&league=${feed.competition.id}&season=${feed.competition.season}`).then((response) => response.json()).then((data: TeamAnalysis) => setTeamAnalysis(data)).catch(() => setTeamAnalysis({ connected: false, error: 'Team analysis could not be reached.' })).finally(() => setTeamLoading(false));
  };
  const loadMatch = (fixtureId: number) => {
    setMatchOpen(true); setMatchLoading(true); setMatchAnalysis(null);
    fetch(`/api/football/match?fixture=${fixtureId}`).then((response) => response.json()).then((data: MatchAnalysis) => setMatchAnalysis(data)).catch(() => setMatchAnalysis({ connected: false, error: 'Match analysis could not be reached.' })).finally(() => setMatchLoading(false));
  };
  ...
  </section><TeamAnalysisDialog open={teamOpen} onOpenChange={setTeamOpen} data={teamAnalysis} loading={teamLoading} /><MatchAnalysisDialog open={matchOpen} onOpenChange={setMatchOpen} data={matchAnalysis} loading={matchLoading} /></>;
}
